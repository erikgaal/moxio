<x-base>
    <main class="w-screen h-screen bg-gradient-to-tr from-[#7fbdea] to-[#9980fc] flex items-center justify-center p-6">
        <x-card class="max-w-screen-lg bg-sky-100/20 flex-grow">
            <x-table class="table-fixed">
                ...
                <x-table-body>
                    <tr :foreach="$concepts as $concept">
                        <x-table-cell class="truncate">{{ $concept->naam }}</x-table-cell>
                        <x-table-cell class="flex flex-wrap gap-1">
                            <x-badge class="truncate" :foreach="$concept->subtypen as $subtype">{{ $subtype->naam }}</x-badge>
                        </x-table-cell>
                        <x-table-cell>{{ count($concept->getTransitiveSubtypes()) }}</x-table-cell>
                    </tr>
                </x-table-body>
            </x-table>
